# thinktank LAN 수신기 기동 (모드1: 같은 WiFi 에서 폰 -> PC 직접 전송)
#
...
#   .\scripts\run_receiver.ps1 -IngestDir D:/thinktank-lan   # 격리 테스트
...
if ($IngestDir) {
    ...
    $shownDir = if ($envLine) { ($envLine.Line -split '=', 2)[1].Trim() + "  (.env)" } else { "~/.thinktank/inbox  (기본값)" }
}
...
$scheme = "http"
if ($Tls) {
    $certFile = Join-Path $HOME ".thinktank\receiver-cert.pem"
    # 핀은 stdout 으로 나온다. 로그는 stderr 라 버린다 — 합치면(2>&1)
    # ErrorActionPreference=Stop 이 NativeCommandError 로 잡아 죽인다.
    $pin = & "$repo\.venv\Scripts\python.exe" -m thinktank.receiver `
...
# 로컬 사본(~/.thinktank)을 둔다. 다른 경로를 쓰려면 RECEIVER_APK 를 직접 지정.
if (-not $env:RECEIVER_APK) {
    $apk = Join-Path $HOME ".thinktank\thinktank-recorder.apk"
...
if ($LogFile) {
    ...
    & "$repo\.venv\Scripts\python.exe" -m thinktank.receiver --host $BindAddress *>> $LogFile
} else {
    & "$repo\.venv\Scripts\python.exe" -m thinktank.receiver --host $BindAddress
}
