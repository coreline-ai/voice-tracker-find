...
하면 기존 :func:`airvoice.main.run_pipeline` 이 상태머신 기반으로 알아서
집어간다. 다만 :func:`airvoice.ingest.scan_ingest_folder` 는 최종 파일명이
보이면 전송 완료로 간주하므로, 업로드 중에는 스캔 대상이 아닌 임시 이름
(``.airvoice.{name}.part``)으로 받고 완료 시점에 :func:`os.replace` 로
원자적 교체한다 (Syncthing 이 하던 것과 같은 보장).

여러 사람을 받을 수 있다. ``~/.airvoice/users.json`` 이 있으면 사용자마다
...
from airvoice.config import Settings, load_settings
from airvoice.ingest import AUDIO_EXTENSIONS
from airvoice.adapters.local_receiver import LocalReceiverV1Adapter
from airvoice.receiver_v1 import (
    V1Error,
    etag_for,
    normalize_sha256,
)
...
from airvoice.web_dashboard import build_dashboard_summary
...
def _is_safe_name(name: str) -> bool:
    ...
    - 앞뒤 점/공백 — 윈도우가 조용히 잘라내므로 다른 파일과 충돌시킬 수 있다.
      선행 점을 막으면 내부 임시 파일(``.airvoice.*.part``) 흉내도 함께 막힌다.
...
_TRIGGER_LOG = Path("~/.airvoice/pipeline-trigger.log").expanduser()
    ...
    try:
        ...
        with _TRIGGER_LOG.open("a", encoding="utf-8") as log:
            subprocess.Popen(  # noqa: S603 - 고정 명령, user_name 은 users.json 로 검증됨
                [sys.executable, "-m", "airvoice.main", "--user", user_name],
                cwd=str(_REPO_ROOT),
                stdout=log,
...
class _Handler(BaseHTTPRequestHandler):
    ...
    server_version = "airvoice-receiver"
    ...
    def _handle_dashboard_asset(self, segments: list[str]) -> None:
        ...
        asset_types = {
            ...
            "token-storage.js": "text/javascript; charset=utf-8",
    ...
    def _handle_upload(self, user, filename: str) -> None:  # noqa: ANN001
        ...
        part = ingest_dir / f".airvoice.{filename}.part"
        remaining = length
        try:
...
def create_server(
    *,
    ...
    Args:
        users: :class:`airvoice.users.User` 목록. 사용자마다 토큰과 전용 경로
...
def ensure_cert(certfile: str | Path, host: str) -> str:
    ...
    name = x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, "airvoice-receiver")])
...
def main() -> None:
    """`python -m airvoice.receiver` 진입점."""
    parser = argparse.ArgumentParser(description="AI R Voice LAN 수신기")
