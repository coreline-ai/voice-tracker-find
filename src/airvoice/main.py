...
주입받으며, 인자가 None 이면 `python -m airvoice.main` 실행 경로(또는 호출자가
...
from airvoice.emerge import EmergeFn, load_cli_emerger, load_emerger, run_emerge
...
from airvoice.ingest import sync_ingest_folder
from airvoice.lock import AlreadyRunning, PipelineLock
from airvoice.notes.archive import Segment, Transcript, write_archive_note
from airvoice.notes.daily import write_daily_note
from airvoice.notes.emerged import IDEAS_SUBDIR, write_emerged_notes
from airvoice.notes.recording_memo import write_recording_memo
...
from airvoice.transcribe import TranscribeFn, load_transcriber, run_transcribe_batch
from airvoice.users import load_users
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="AI R Voice 배치 파이프라인 실행")
    parser.add_argument(
        "--force-emerge",
        action="store_true",
...
def main(argv: list[str] | None = None) -> PipelineRun | None:
    ...
    for user in users:
        ...
        try:
            ...
            if args.normalize:
                from airvoice.vault_incremental import (
                    _robust_cli_llm,
