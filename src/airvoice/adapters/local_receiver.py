"""Local file/SQLite persistence adapter for the versioned receiver API.

The legacy Android client is intentionally kept in :mod:`airvoice.receiver`.
...
def cleanup_stale_upload_parts(
    ...
    *,
    ...
    for directory in directories:
        try:
            candidates = tuple(directory.glob(".airvoice-v1.*.part"))
        except OSError:
            continue
...
def has_room_for(directory: Path, incoming: int) -> bool:
    """Return whether a local ingest directory keeps its safety reserve."""
    try:
...
def atomic_write(path: Path, content: bytes) -> None:
    part = path.parent / f".airvoice-v1.{uuid.uuid4().hex}.part"
    try:
...
class LocalReceiverV1Adapter:
    """Thread-safe local persistence adapter shared by handler instances."""
    ...
    def ensure_upload_capacity(user, length: int) -> None:  # noqa: ANN001
        """Reject uploads that would consume the local pipeline's reserve."""
        if not has_room_for(user.settings.ingest_dir, length):
    ...
    def receive_upload(
        ...
        *,
        ...
        temp = ingest_dir / f".airvoice-v1.{uuid.uuid4().hex}.part"
        remaining = length
        digest = hashlib.sha256()
        try:
            ...
            with self._write_lock, self._connect() as db:
                ...
                if existing is not None:
                    if not receipt_matches(existing, upload):
                        raise V1Error(
                            409,
                            "UPLOAD_CONFLICT",
