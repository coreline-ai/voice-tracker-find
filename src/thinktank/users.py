...
``~/.thinktank/users.json`` 이 있으면 다중 사용자 모드, 없으면 기존 ``.env``
...
users.json 형식::
    ...
    [
      {"name": "user1", "token": "...", "ingest_dir": "D:/thinktank",
       "vault": "~/thinktank-vault", "db_path": "~/.thinktank/db/pipeline.db",
       "temp_dir": "~/.thinktank/temp"},
      {"name": "user2", "token": "..."}
    ]

``name`` 과 ``token`` 만 필수다. 경로를 생략하면
``~/.thinktank/users/{name}/`` 아래로 자동 배치한다.
"""
...
USERS_FILE = "~/.thinktank/users.json"

# 경로를 생략한 사용자의 기본 배치 위치.
_USER_BASE = "~/.thinktank/users"
